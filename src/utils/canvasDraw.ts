...
import { computedItemShowArea, isVideo } from '@/utils/common.ts'
...
export class CanvasPlayer {
	...
	initWatch() {
		...
		watch(
			[
				this.attrStore.trackAttrMap,
				() => this.playerStore.playTargetTrackMap,
				() => this.canvasSize
			],
			throttle(async () => this.drawCanvas(), 30),
			{ deep: true }
		)
		// 容器大小变化
		watch(
		...
		// 前置依赖加载完成
		watch(
			[this.ffmpeg.isLoaded, () => this.playerStore.ingLoadingCount],
			() => this.initPlayer(),
			{
				flush: 'post',
				immediate: true
	...
	updateCanvasSize({ width, height }: Record<string, number>) {
		...
		const scaleWidth =
			playerWidth !== 0
				? Math.floor((height / playerHeight) * playerWidth)
				: width // 等高情况下的宽度
		const scaleHeight =
			playerHeight !== 0
				? Math.floor((width / playerWidth) * playerHeight)
				: height // 等宽情况啊下的高度
	...
	async drawCanvas() {
		...
		this.playerStore.playTargetTrackMap.forEach(
			(trackItem: Record<string, any>, id: number) => {
	...
	drawToRenderCanvas(
		...
		return new Promise((resolve) => {
			const { sourceWidth, sourceHeight, drawL, drawT, drawW, drawH } =
				this.computedRect(trackItem, id)
			...
			if (frameIndex > end) {
				...
				const blobFrame = this.ffmpeg.getGifFrame(
					name,
