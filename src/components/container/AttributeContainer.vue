<template>
  <div class="select-none relative pl-2" :style="attrWidth">
    ...
    <div
      v-if="selectTrackOptionsConfig.length > 0"
      class="absolute top-0 left-3 right-2 bottom-0 overflow-hidden"
    >
      <AttrContainer
...
<script setup lang="ts">
  import { AttrContainer, SplitLine } from '@/components'
  ...
  // 将data下的配置导入
  const attributeFiles = import.meta.glob('@/data/options/*.ts', {
    eager: true
  })
