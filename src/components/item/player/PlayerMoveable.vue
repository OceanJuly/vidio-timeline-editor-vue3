...
<script setup lang="ts">
  ...
  const targetList = computed(() => {
    ...
    store.playTargetTrackMap.forEach((trackItem, id) => {
      ...
      let { drawL, drawT, diffW, diffH, defaultH, defaultW } =
        computedItemShowArea(
          trackItem,
          { width, height },
          attrStore.trackAttrMap[id]
        )
      if (left === 0 && top === 0 && scale === 100) {
        // 初次渲染
        drawL = diffW / 2
  ...
  const onScale = (params: Record<string, any>) => {
