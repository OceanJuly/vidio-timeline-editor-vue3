import { inject, reactive, ref, toRaw, watch, watchPostEffect } from 'vue'
...
export function audioSetup(ffmLoading: Ref<boolean>) {
  ...
  async function initAudio() {
    ...
    if (
      ffmpeg.isLoaded.value &&
      store.ingLoadingCount === 0 &&
      !ffmLoading.value
    ) {
