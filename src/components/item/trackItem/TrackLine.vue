<template>
  <div
    ...
    :class="[
      ...
      isActive ? 'dark:bg-gray-700 bg-gray-100 bg-opacity-20' : '',
  ...
  >
    <template v-for="(item, index) of lineData" :key="item.id">
      <TrackItemDetail
        ...
        :item-index="index"
        :track-item="item as Record<string, any>"
        draggable="true"
        @dragstart="
          dragLineHandler($event, 'start', lineIndex, index, item.type)
        "
      />
    </template>
  </div>
...
<script setup lang="ts">
  ...
  import { TrackItemDetail } from '@/components'
  ...
  const dragLineHandler = (
    ...
    if (type === 'start') {
      ...
      store.dragData.dataInfo = JSON.stringify(
        store.trackList[lineIndex].list[index]
      )
