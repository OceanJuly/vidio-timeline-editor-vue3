<template>
  ...
  <div class="formItem" v-else-if="componentData.dataType === 'Slider'">
    ...
    <span class="ml-2 w-12 text-center text-sm leading-8"
      >{{ formValue }}{{ componentData.label }}</span
    >
  </div>
...
<script setup lang="ts">
  import { ColorPicker, AttrContainer } from '@/components'
  ...
  const formValue = computed({
    get() {
      if (selectTrack.value && attrStore.trackAttrMap[selectTrack.value.id]) {
        const attrData =
          attrStore.trackAttrMap[selectTrack.value.id][
            props.componentData.mappingKey
          ]
        if (
          attrData === undefined &&
          props.componentData.defaultValue !== undefined
        ) {
