...
import { useTrackAttr } from '@/store/trackAttr.ts'
import type { TrackItem, TrackLineItem } from '@/types'
import { getJsonParse } from '@/utils/common.ts'
import { checkTrackListOverlap } from '@/utils/track.ts'

export const useTrackState = defineStore('trackState', () => {
	const trackAttrStore = useTrackAttr()
	...
	const trackList = reactive<TrackLineItem[]>(
		localStorage.trackList ? (getJsonParse(localStorage.trackList) as any) : []
	)
	// 选中元素坐标
	...
	const removeTrack = (
		...
		removeAttr && trackAttrStore.deleteTrack(removeItem.id)
	}

	// 复用已有行
	const insertExistingLine = (
		item: TrackItem,
		insertLine: { line: number; index: number }
	) => {
	...
	// 插入新行
	const insertNewLine = (item: TrackItem) => {
	...
	// 移动目标行
	const moveTargetLine = (
		item: TrackItem,
		insertLine: { line: number; index: number }
	) => {
		let { lineIndex: moveLineIndex = -1, itemIndex: moveIndex = -1 } =
			moveTrackData
		trackList[insertLine.line].list.splice(
			insertLine.index,
			moveLineIndex === insertLine.line ? 1 : 0,
			item
		)
		if (moveLineIndex !== -1 && insertLine.line !== moveLineIndex) {
			if (
				trackList[moveLineIndex].list.length === 1 &&
				insertLine.line > moveLineIndex
			) {
		...
		selectTrackItem.index =
			moveLineIndex === -1 ? insertLine.index + 1 : insertLine.index
	}

	// 目标行不可用，则移动到目标之后、之前
	const moveLine = (item: TrackItem, targetLineIndex: number) => {
		let { lineIndex: moveLineIndex = -1, itemIndex: moveIndex = -1 } =
			moveTrackData
		...
		if (moveLineIndex !== -1 && moveIndex !== -1) {
			// 移动到新行，删除老数据
	...
	// 新增轨道
	const addTrack = (item: TrackItem, lineIndex = -1, insertBefore = true) => {
		const { type } = item
		...
		for (
			...
			const { hasOverlap, insertIndex } = checkTrackListOverlap(
				lineData.list,
				item,
				moveLineIndex === i ? moveIndex : -1
			)
			if (lineData.type === type && !hasOverlap) {
				// 存在可复用行
				canInsertLines.push({
					line: i,
					index: insertIndex
		...
		if (lineIndex !== -1) {
			// 指定行插入, 注意轨道渲染顺序 0 在最上方
			let targetLineIndex = insertBefore ? lineIndex : lineIndex + 1
			let insertLine = canInsertLines.find(
				(canInsItem) => canInsItem.line === lineIndex
			)
			if (insertLine) {
				// 目标行可插入
		...
		} else if (canInsertLines.length > 0) {
			// 左侧列表添加，可复用现有同类型行内空隙
		...
		trackAttrStore.initTrackAttr(item.id)
	}
