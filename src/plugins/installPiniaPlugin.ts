...
class StepManager {
  ...
  private readonly stateList: StepData[] = []
 ...
 * 初始化原始数据
 * */
const initOriginState = (store: Store, stepManager: StepManager) => {
  ...
  if (WatchKeys[storeId]) {
    ...
    WatchKeys[storeId].forEach((key: string) => {
      ...
      if (isRef(value)) {
        // 基础数据保存副本，复杂数据通过oldValue获取更改前数据
...
const actionHandler = ({ name, store, args, after, onError }) => {
...
const StoreStateHook = ({ store }: PiniaPluginContext) => {
  ...
  const redo = () => {
    ...
    if (
      ...
      stepManagerIns.statePoint.value !== stepManagerIns.stateLength.value
    ) {
