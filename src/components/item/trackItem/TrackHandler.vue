<template>
	...
	>
		<div
			class="cursor-w-resize flex flex-col justify-center absolute bottom-0 -top-px -left-2 text-center rounded-tl rounded-bl w-2 dark:bg-gray-100 bg-gray-600 dark:text-gray-800 text-gray-100"
			ref="handlerLeft"
		...
		<div
			class="cursor-e-resize flex flex-col justify-center absolute bottom-0 -top-px -right-2 text-center rounded-tr rounded-br w-2 dark:bg-gray-100 bg-gray-600 dark:text-gray-800 text-gray-100"
			ref="handlerRight"
...
<script setup lang="ts">
	...
	const frameWidth = computed(() => getGridPixel(trackStore.trackScale, 1))
	const targetTrack = computed(() => {
		return trackStore.trackList[props.lineIndex].list[props.itemIndex]
	})
	...
	const setTrackFrameData = (frameCount: number, handleType: string) => {
		...
		if (handleType === 'left') {
			...
			if (isVA) {
				...
				trackStore.trackList[props.lineIndex].list[props.itemIndex].offsetL =
					Math.max(offsetL + diffStart, 0)
			} else {
				// 其他资源操作无限制
				trackStore.trackList[props.lineIndex].list[props.itemIndex].frameCount =
					originEnd - newStart
			}
			trackStore.trackList[props.lineIndex].list[props.itemIndex].start =
				newStart
		} else {
			// 右侧手柄
			...
			if (isVA) {
				...
				trackStore.trackList[props.lineIndex].list[props.itemIndex].offsetR =
					Math.max(offsetR - diffEnd, 0)
			} else {
				// 其他资源操作无限制
				trackStore.trackList[props.lineIndex].list[props.itemIndex].frameCount =
					newEnd - originStart
			}
			trackStore.trackList[props.lineIndex].list[props.itemIndex].end = newEnd
		}
	}
	...
	const initLimits = (lineData: TrackItem[], trackItem: TrackItem) => {
		...
		const afterTrack =
			props.itemIndex < lineData.length ? lineData[props.itemIndex + 1] : null
	...
	const mouseDownHandler = (e: MouseEvent, type: string) => {
		...
		enableMove = true
		initLimits(
			trackStore.trackList[props.lineIndex]?.list || [],
			targetTrack.value
		)
