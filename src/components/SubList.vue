  ...
  <div class="w-full flex flex-col pt-2 pl-2 pr-1 mt-2">
    ...
    <ul class="flex flex-row flex-wrap">
      <li
        ...
        v-for="(item, _) of listData.items"
        :key="`${item.name}${item.cover}${_}`"
      >
        <template v-if="isAudio">
          <AudioResourceItem :data="item" :type="type" />
        </template>
        <template v-else>
          <OtherResourceItem :data="item" :type="type" />
        </template>
      </li>
    </ul>
  </div>
...
<script setup lang="ts">
  import { AudioResourceItem, OtherResourceItem } from '@/components'
