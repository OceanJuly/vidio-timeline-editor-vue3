...
module.exports = {
  ...
  rules: {
    ...
    'no-unused-private-class-members': IS_PROD ? 'error' : 'off', // 允许Class中有未使用的属性或方法成员，存在开发人员预先定义结构的场景。 发布前校验
    ...
    'class-methods-use-this': IS_PROD ? 'error' : 'off', // 强制类方法使用this,如果一个类方法不使用this，有时可以做成静态函数。 发布前校验
    ...
    'no-console': IS_PROD ? 'error' : 'off', // 生产环境不能有console,开发环境可以有
    ...
    'no-debugger': IS_PROD ? 'error' : 'off', // 禁用 debugger
