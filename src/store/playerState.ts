...
import type { TrackItem, VideoTractItem } from '@/types'

export const usePlayerState = defineStore('playerState', () => {
	const trackStore = useTrackState()
	const attrStore = useTrackAttr()
	...
	const mergeVideo = () => {
		...
		trackStore.trackList.forEach((trackLine) => {
			...
			trackLine.list.forEach((trackItem) => {
				...
				if (trackLine.type === 'video') {
					if (playerHeight === 0 && playerWidth === 0) {
						...
						playerWidth = Math.max(
							playerWidth,
							(trackItem as VideoTractItem).width
						)
						playerHeight = Math.max(
							playerHeight,
							(trackItem as VideoTractItem).height
						)
					}
					existV = true
				}
				if (
					!silent &&
					(trackLine.type === 'video' || trackLine.type === 'audio')
				) {
	...
	watch(
		[() => trackStore.trackList, attrStore.trackAttrMap],
		debounce(() => mergeVideo(), 30),
		{
			immediate: true,
			deep: true
		}
